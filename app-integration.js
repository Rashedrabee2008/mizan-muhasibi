...
// app-integration.js (نسخة كاملة - مع تعبئة قوائم التحصيل)
// ============================================================
...
window.getDefaultWarehouse = function() {
    if (typeof warehouses === 'undefined' || !Array.isArray(warehouses) || warehouses.length === 0) return null;
...
window.populateSaleWarehouseSafe = function() {
    ...
    } catch (e) { console.warn('⚠️ خطأ في populateSaleWarehouseSafe:', e.message); }
};

// ═══════════════════════════════════════════════════════════
// ✅ 2. عرض إحصائيات المخازن
// ═══════════════════════════════════════════════════════════
window.renderWarehouseStatsOnDashboard = function() {
    try {
        const grid = document.getElementById('warehouseStatsGrid');
        if (!grid) return;
        if (typeof warehouses === 'undefined' || !Array.isArray(warehouses) || warehouses.length === 0) { grid.innerHTML = '<div class="empty-state" style="grid-column:1/-1;"><i class="fas fa-warehouse"></i><span>لا توجد مخازن</span></div>'; return; }
        if (typeof productWarehouseStock === 'undefined' || !productWarehouseStock || typeof productWarehouseStock !== 'object') window.productWarehouseStock = {};
        let html = '';
        warehouses.slice(0, 4).forEach(function(w) {
            if (!w || !w.id) return;
            let productCount = 0; let totalQty = 0;
            try {
                Object.keys(productWarehouseStock).forEach(function(pid) {
                    ...
                    if (qty && qty > 0) { productCount++; totalQty += qty; }
                });
            } catch (e) {}
            html += '<div class="dashboard-card" style="border-right-color:#4A8AB5;cursor:pointer;" onclick="navigateTo(\'warehouses\')"><div class="number" style="color:#4A8AB5;font-size:16px;">' + (w.name || 'بدون اسم') + (w.isDefault ? ' ⭐' : '') + '</div><div class="label" style="font-size:10px;">📦 ' + productCount + ' صنف<br>📊 ' + totalQty + ' وحدة</div></div>';
        });
        if (html === '') html = '<div class="empty-state" style="grid-column:1/-1;"><i class="fas fa-warehouse"></i><span>لا توجد بيانات</span></div>';
        grid.innerHTML = html;
    } catch (e) { console.warn('⚠️ خطأ في renderWarehouseStatsOnDashboard:', e.message); }
};
...
window.initIntegration = function() {
    try {
        ...
        if (typeof warehouses === 'undefined' || !Array.isArray(warehouses) || warehouses.length === 0) {
            window.warehouses = [{ id: 1, name: 'المخزن الرئيسي', location: 'المركز الرئيسي', isDefault: true, active: true, createdAt: new Date().toISOString() }];
            if (typeof setData === 'function') setData('warehouses', warehouses);
            else localStorage.setItem('mizan_warehouses', JSON.stringify(warehouses));
        }
        if (typeof products === 'undefined' || !Array.isArray(products)) window.products = [];
    ...
    } catch (e) { console.warn('⚠️ خطأ في initIntegration:', e.message); }
};
...
window.addEventListener('DOMContentLoaded', function() {
    setTimeout(function() { try { if (typeof initIntegration === 'function') initIntegration(); } catch (e) {} }, 3000);
    setTimeout(function() { if (typeof renderWarehouseStatsOnDashboard === 'function') { try { renderWarehouseStatsOnDashboard(); } catch(e) {} } }, 4000);
});

// ═══════════════════════════════════════════════════════════
// 🚀 التنقل الآمن (مع تعبئة قوائم التحصيل)
// ═══════════════════════════════════════════════════════════
(function() {
    let _navOriginal = window.navigateTo;
    window.navigateTo = function(page) {
        if (_navOriginal) { try { _navOriginal.apply(this, arguments); } catch (e) {} }
        setTimeout(function() {
            try {
                if (page === 'dashboard') {
                    if (typeof renderWarehouseStatsOnDashboard === 'function') { try { renderWarehouseStatsOnDashboard(); } catch(e) {} }
                }
                if (page === 'cashier') {
                ...
                // ✅ جديد: تعبئة قوائم التحصيل والسداد
                if (page === 'payments') {
...
console.log('✅ تم تحميل app-integration.js بنجاح (مع تعبئة قوائم التحصيل)');
