...
// app-part3.js (نسخة كاملة - مع إصلاح changePassword لـ Firebase)
// ============================================================
...
window.renderTreasury = function() {
    ...
    sorted.forEach(t => {
        ...
        html += `<div class="table-row" style="grid-template-columns: 2.2fr 1fr 1fr 1fr 0.6fr;"><span style="font-size:11px;">${refIcons[t.refType] || '📋'} ${t.note}</span><span style="color:${color};font-weight:700;">${isDep ? '+' : '-'}${formatMoney(t.amount)}</span><span style="color:${color};font-size:10px;">${isDep ? '💚 إيداع' : '❤️ سحب'}</span><span style="font-size:10px;color:#A89070;">${t.date}<br>${t.time || ''}</span><span></span></div>`;
    });
    c.innerHTML = html;
};
...
window.getReturnsCOGS = function(returnsArr) {
    let total = 0;
    (returnsArr || []).forEach(ret => { if (ret.type !== 'sale') return; (ret.items || []).forEach(item => { const p = products.find(pr => pr.id == item.productId); total += (item.costPrice || (p ? p.buy : 0)) * (item.qty || 0); }); });
    return total;
};
...
window.getCustomerBalance = function(customerName) {
    ...
    return Math.max(0, sales.filter(s => s.customer === customerName && s.paymentMethod === 'credit').reduce((sum, s) => sum + (s.remainingAmount !== undefined ? s.remainingAmount : s.total), 0));
};

window.getSupplierBalance = function(supplierName) {
    if (!supplierName) return 0;
    return Math.max(0, purchases.filter(p => p.supplierName === supplierName && p.payment === 'credit').reduce((sum, p) => sum + (p.remainingAmount !== undefined ? p.remainingAmount : p.total), 0));
};
...
window.resetCustomerForm = function() {
    $('customerId').value = ''; $('customerName').value = ''; $('customerPhone').value = ''; $('customerWhatsapp').value = ''; $('customerAddress').value = '';
...
window.renderCustomers = function() {
    ...
    filtered.forEach(cu => {
        ...
        html += `<div class="table-row" style="grid-template-columns: 1.5fr 1.2fr 1fr ${showActions ? '1.8fr' : '0'};"><span><strong>${cu.name}</strong></span><span style="font-size:11px;">${cu.phone || '-'}</span><span style="color:${balance > 0 ? '#E06060' : '#2D8F5E'};font-weight:900;">${formatMoney(balance)}</span>${showActions ? `<div style="display:flex;gap:4px;"><button class="btn btn-info btn-sm" onclick="viewCustomerStatement('${cu.name}')"><i class="fas fa-file-invoice-dollar"></i></button>${canEdit() ? `<button class="btn btn-warning btn-sm" onclick="editCustomer(${cu.id})"><i class="fas fa-edit"></i></button>` : ''}${canDelete() ? `<button class="btn btn-danger btn-sm" onclick="deleteCustomer(${cu.id})"><i class="fas fa-trash"></i></button>` : ''}</div>` : ''}</div>`;
    });
    c.innerHTML = html;
};
...
window.viewCustomerStatement = function(customerName) {
    ...
    const html = `<button class="modal-close" onclick="closeModal()">&times;</button><h3>📋 كشف حساب: ${customerName}</h3><div class="invoice-print"><div class="inv-header"><h2>${companyData.name || 'الميزان'}</h2><p>كشف حساب</p></div><div class="inv-info"><div>الرصيد: ${formatMoney(balance)} 🇪🇬</div></div></div><div style="display:flex;gap:6px;margin-top:12px;flex-wrap:wrap;"><button class="btn btn-primary" onclick="window.print()" style="flex:1;"><i class="fas fa-print"></i> طباعة</button>${hasWhatsapp && balance > 0 && typeof remindCustomerWhatsApp === 'function' ? `<button class="whatsapp-btn" onclick="remindCustomerWhatsApp('${customerName}')" style="flex:1;justify-content:center;"><i class="fab fa-whatsapp"></i> تذكير</button>` : ''}<button class="btn btn-secondary" onclick="closeModal()" style="flex:1;"><i class="fas fa-times"></i> إغلاق</button></div>`;
    openModal(html);
};
...
window.saveSupplier = function() {
    ...
    else { if (suppliers.find(s => s.name === name)) { showToast('⚠️ المورد موجود', 'warning'); return; } suppliers.push({ id: Date.now(), name, phone, whatsapp, address, createdAt: new Date().toISOString() }); showToast('✅ تم إضافة المورد', 'success'); }
...
window.editSupplier = function(id) {
    ...
    $('supplierId').value = s.id; $('supplierName').value = s.name; $('supplierPhone').value = s.phone || ''; $('supplierWhatsapp').value = s.whatsapp || ''; $('supplierAddress').value = s.address || '';
...
window.resetSupplierForm = function() {
    $('supplierId').value = ''; $('supplierName').value = ''; $('supplierPhone').value = ''; $('supplierWhatsapp').value = ''; $('supplierAddress').value = '';
...
window.renderSuppliers = function() {
    ...
    filtered.forEach(s => {
        const balance = getSupplierBalance(s.name);
        html += `<div class="table-row" style="grid-template-columns: 1.5fr 1.2fr 1fr ${showActions ? '1.8fr' : '0'};"><span><strong>${s.name}</strong></span><span style="font-size:11px;">${s.phone || '-'}</span><span style="color:${balance > 0 ? '#E6A830' : '#2D8F5E'};font-weight:900;">${formatMoney(balance)}</span>${showActions ? `<div style="display:flex;gap:4px;"><button class="btn btn-info btn-sm" onclick="viewSupplierStatement('${s.name}')"><i class="fas fa-file-invoice"></i></button>${canEdit() ? `<button class="btn btn-warning btn-sm" onclick="editSupplier(${s.id})"><i class="fas fa-edit"></i></button>` : ''}${canDelete() ? `<button class="btn btn-danger btn-sm" onclick="deleteSupplier(${s.id})"><i class="fas fa-trash"></i></button>` : ''}</div>` : ''}</div>`;
    });
    c.innerHTML = html;
};
...
window.viewSupplierStatement = function(supplierName) {
    ...
    const html = `<button class="modal-close" onclick="closeModal()">&times;</button><h3>📋 كشف حساب: ${supplierName}</h3><div class="invoice-print"><div class="inv-header"><h2>${companyData.name || 'الميزان'}</h2><p>كشف حساب مورد</p></div><div class="inv-info"><div>الرصيد: ${formatMoney(balance)} 🇪🇬</div></div></div><div style="display:flex;gap:6px;margin-top:12px;flex-wrap:wrap;"><button class="btn btn-primary" onclick="window.print()" style="flex:1;"><i class="fas fa-print"></i> طباعة</button>${hasWhatsapp && balance > 0 && typeof remindSupplierWhatsApp === 'function' ? `<button class="whatsapp-btn" onclick="remindSupplierWhatsApp('${supplierName}')" style="flex:1;justify-content:center;"><i class="fab fa-whatsapp"></i> إشعار</button>` : ''}<button class="btn btn-secondary" onclick="closeModal()" style="flex:1;"><i class="fas fa-times"></i> إغلاق</button></div>`;
    openModal(html);
};
...
window.populateCollectCustomers = function() {
    ...
    if (typeof customers !== 'undefined' && Array.isArray(customers) && customers.length > 0) {
        customers.forEach(function(c) {
            ...
            const balanceText = balance > 0 ? ' (مديونية: ' + formatMoney(balance) + ')' : '';
            sel.innerHTML += '<option value="' + c.name + '">' + c.name + balanceText + '</option>';
        });
    } else {
        sel.innerHTML += '<option value="" disabled>لا يوجد عملاء مسجلون</option>';
    }
    sel.value = cv;
    if (typeof _applyAllSearch === 'function') setTimeout(_applyAllSearch, 50);
};
...
window.populatePaySuppliers = function() {
    ...
    if (typeof suppliers !== 'undefined' && Array.isArray(suppliers) && suppliers.length > 0) {
        suppliers.forEach(function(s) {
            const balance = typeof getSupplierBalance === 'function' ? getSupplierBalance(s.name) : 0;
            const balanceText = balance > 0 ? ' (مديونية: ' + formatMoney(balance) + ')' : '';
            sel.innerHTML += '<option value="' + s.name + '">' + s.name + balanceText + '</option>';
        });
    } else {
        sel.innerHTML += '<option value="" disabled>لا يوجد موردون مسجلون</option>';
    }
    sel.value = cv;
    if (typeof _applyAllSearch === 'function') setTimeout(_applyAllSearch, 50);
};
...
window.populateSettleCustomers = function() {
    const sel = document.getElementById('settleCustomer'); if (!sel) return;
    const cv = sel.value;
    sel.innerHTML = '<option value="">اختر عميل...</option>';
    if (typeof customers !== 'undefined' && Array.isArray(customers)) {
        customers.forEach(c => {
            const bal = getCustomerBalance(c.name);
            sel.innerHTML += `<option value="${c.name}">${c.name}${bal > 0 ? ' (مديونية: ' + formatMoney(bal) + ')' : ''}</option>`;
        });
    }
    sel.value = cv;
    if (typeof _applyAllSearch === 'function') setTimeout(_applyAllSearch, 50);
...
window.populateSettleProducts = function() {
    const sel = document.getElementById('settleProduct'); if (!sel) return;
    const cv = sel.value;
...
window.switchPayTab = function(tab, btn) {
    ...
    const settle = document.getElementById('payTabSettle');
    ...
    if (tab === 'collect') { if (typeof populateCollectCustomers === 'function') { try { populateCollectCustomers(); } catch(e) {} } }
    if (tab === 'pay') { if (typeof populatePaySuppliers === 'function') { try { populatePaySuppliers(); } catch(e) {} } }
    if (tab === 'settle') {
        if (typeof populateSettleCustomers === 'function') { try { populateSettleCustomers(); } catch(e) {} }
        if (typeof populateSettleProducts === 'function') { try { populateSettleProducts(); } catch(e) {} }
    }
};
...
window.renderPayments = function() {
    ...
    sorted.forEach(p => {
        ...
        html += `<div class="table-row" style="grid-template-columns: 0.8fr 1fr 1.5fr 1fr 0.8fr;"><span style="color:${isCollect ? '#2D8F5E' : '#E06060'};font-weight:700;font-size:11px;">${isCollect ? '💰 تحصيل' : '💸 سداد'}</span><span style="font-size:10px;color:#A89070;">${p.date}</span><span style="font-size:11px;">${p.party}</span><span style="color:${isCollect ? '#2D8F5E' : '#E06060'};font-weight:700;">${formatMoney(p.amount)}</span><div><button class="btn btn-info btn-sm" onclick="showReceiptById(${p.id})"><i class="fas fa-receipt"></i></button></div></div>`;
    });
    c.innerHTML = html;
};
...
window.showReceipt = function(pay) {
    ...
    const html = `<button class="modal-close" onclick="closeModal()">&times;</button><h3>🧾 ${label}</h3><div class="receipt-print"><div class="rec-header"><h2>${companyData.name || 'الميزان'}</h2><p>${label}</p></div><div class="rec-info"><div><span class="lbl">رقم الإيصال:</span><span>#${pay.id.toString().slice(-6)}</span></div><div><span class="lbl">التاريخ:</span><span>${pay.date}</span></div><div><span class="lbl">الوقت:</span><span>${pay.time}</span></div><div><span class="lbl">${isCollect ? 'العميل' : 'المورد'}:</span><span>${pay.party}</span></div></div><div class="rec-amount"><div class="lbl">${isCollect ? 'المبلغ المستلم' : 'المبلغ المدفوع'}</div><div class="value">${formatMoney(pay.amount)} ج.م</div></div><div class="rec-footer">${companyData.footer || 'شكراً لتعاملكم معنا 🌟'}</div></div><div style="display:flex;gap:6px;margin-top:12px;flex-wrap:wrap;"><button class="btn btn-primary" onclick="window.print()" style="flex:1;"><i class="fas fa-print"></i> طباعة</button>${hasWhatsapp && typeof sendReceiptWhatsApp === 'function' ? `<button class="whatsapp-btn" onclick="sendReceiptWhatsApp(${pay.id})" style="flex:1;justify-content:center;"><i class="fab fa-whatsapp"></i> واتساب</button>` : ''}<button class="btn btn-secondary" onclick="closeModal()" style="flex:1;"><i class="fas fa-times"></i> إغلاق</button></div>`;
    openModal(html);
};
...
window.renderInvoices = function() {
    ...
    sorted.forEach(inv => { 
        ...
        html += `<div class="table-row" style="grid-template-columns: 0.5fr 1.3fr 1fr 0.7fr 1.3fr;"><span>#${inv.number}</span><span>${inv.customer}</span><span style="color:#2D8F5E;font-weight:700;">${formatMoney(inv.total)}</span><span>${inv.invoiceType === 'tax' ? '🧾' : '📋'} ${statusLabel}</span><div style="display:flex;gap:4px;"><button class="btn btn-info btn-sm" onclick="showSaleInvoiceDetails(${inv.id})"><i class="fas fa-eye"></i></button><button class="btn btn-danger btn-sm" onclick="deleteInvoice(${inv.id})"><i class="fas fa-trash"></i></button></div></div>`; 
    });
    c.innerHTML = html;
};
...
window.deleteJournalEntry = function(id) { if (!canDelete()) return; if (!confirm('⚠️ حذف القيد؟')) return; window.journalEntries = journalEntries.filter(e => e.id !== id); setData('journalEntries', journalEntries); renderJournal(); renderAccounts(); showToast('🗑️ تم الحذف', 'info'); };
...
window.switchAccountsTab = function(tab, btn) {
    ...
    if (btn) btn.classList.add('active');
    const tree = document.getElementById('accTabTree');
    const journal = document.getElementById('accTabJournal');
    const reports = document.getElementById('accTabReports');
    ...
    if (tab === 'tree') {
        if (typeof renderAccounts === 'function') { try { renderAccounts(); } catch(e) {} }
        if (typeof populateAccountDropdowns === 'function') { try { populateAccountDropdowns(); } catch(e) {} }
    }
    if (tab === 'journal') {
...
window.renderReport = function(type) {
    ...
    container.innerHTML = `
        ...
        <div class="report-summary">
            ...
            <div class="report-stat"><div class="num" style="color:${tPr >= 0 ? '#2D8F5E' : '#E06060'};">${formatMoney(tPr)}</div><div class="lbl">📈 ربح</div></div>
        </div>
        <div class="report-chart">
            <h4>📊 الرسم البياني</h4>
            <div class="chart-bars">
                ${rows.map(r => `<div class="chart-bar-wrap"><div style="display:flex;gap:2px;width:100%;align-items:flex-end;height:100%;justify-content:center;"><div class="chart-bar sales" style="height:${Math.max((r.sales / maxV) * 100, 3)}%;width:33%;">${r.sales > 0 ? `<span class="chart-bar-value">${r.sales.toFixed(0)}</span>` : ''}</div><div class="chart-bar purchases" style="height:${Math.max((r.cogs / maxV) * 100, 3)}%;width:33%;">${r.cogs > 0 ? `<span class="chart-bar-value">${r.cogs.toFixed(0)}</span>` : ''}</div><div class="chart-bar expenses" style="height:${Math.max((r.expenses / maxV) * 100, 3)}%;width:33%;">${r.expenses > 0 ? `<span class="chart-bar-value">${r.expenses.toFixed(0)}</span>` : ''}</div></div><div class="chart-bar-label">${r.label}</div></div>`).join('')}
            </div>
            <div class="report-legend"><span><span class="dot" style="background:#2D8F5E;"></span> مبيعات</span><span><span class="dot" style="background:#E06060;"></span> تكلفة</span><span><span class="dot" style="background:#E6A830;"></span> مصروفات</span></div>
        </div>
        ...
        ${rows.map(r => `<div class="table-row" style="grid-template-columns: 1.3fr 1fr 1fr 1fr 1fr 1fr;font-size:11px;"><span>${r.label}</span><span style="color:#2D8F5E;">${formatMoney(r.sales)}</span><span style="color:#E06060;">${formatMoney(r.cogs)}</span><span style="color:#E6A830;">${formatMoney(r.expenses)}</span><span style="color:#C9A94E;">${formatMoney(r.returns)}</span><span style="color:${r.profit >= 0 ? '#2D8F5E' : '#E06060'};font-weight:700;">${formatMoney(r.profit)}</span></div>`).join('')}
    `;
};
...
window.renderVATReport = function() {
    ...
    container.innerHTML = `
        ...
        <div class="vat-report-card"><div class="vat-big">${formatMoney(totalVATDue)} 🇪🇬</div><div class="vat-label">${totalVATDue >= 0 ? '💰 ضريبة مستحقة' : '✅ ضريبة مستردة'}</div></div>
        <div class="report-summary"><div class="report-stat" style="border-right-color:#9B59B6;"><div class="num" style="color:#9B59B6;">${formatMoney(totalSalesVAT)}</div><div class="lbl">🧾 ضريبة المبيعات</div></div><div class="report-stat" style="border-right-color:#E06060;"><div class="num" style="color:#E06060;">${formatMoney(totalPurchasesVAT)}</div><div class="lbl">🛒 ضريبة المشتريات</div></div></div>
        <div class="report-chart"><h4>📊 الرسم البياني الشهري</h4><div class="chart-bars">${rows.map(r => `<div class="chart-bar-wrap"><div style="display:flex;gap:2px;width:100%;align-items:flex-end;height:100%;justify-content:center;"><div class="chart-bar vat" style="height:${Math.max((r.salesVAT / maxV) * 100, 3)}%;width:48%;">${r.salesVAT > 0 ? `<span class="chart-bar-value">${r.salesVAT.toFixed(0)}</span>` : ''}</div><div class="chart-bar purchases" style="height:${Math.max((r.purchasesVAT / maxV) * 100, 3)}%;width:48%;">${r.purchasesVAT > 0 ? `<span class="chart-bar-value">${r.purchasesVAT.toFixed(0)}</span>` : ''}</div></div><div class="chart-bar-label">${r.label}</div></div>`).join('')}</div><div class="report-legend"><span><span class="dot" style="background:#9B59B6;"></span> ضريبة مبيعات</span><span><span class="dot" style="background:#E06060;"></span> ضريبة مشتريات</span></div></div>
        ...
        ${rows.map(r => `<div class="table-row" style="grid-template-columns: 1.5fr 1fr 1fr 1fr;font-size:11px;"><span>${r.label}</span><span style="color:#9B59B6;">${formatMoney(r.salesVAT)}</span><span style="color:#E06060;">${formatMoney(r.purchasesVAT)}</span><span style="color:${r.vatDue >= 0 ? '#E6A830' : '#2D8F5E'};font-weight:700;">${formatMoney(r.vatDue)}</span></div>`).join('')}
    `;
};
...
window.saveUser = function() {
    ...
    setData('users', users); resetUserForm(); renderUsers();
};
...
window.toggleUserActive = function(id) { const u = users.find(us => us.id == id); if (!u) return; if (u.id === (currentUser ? currentUser.id : null)) { showToast('⚠️ لا يمكنك تعطيل حسابك', 'error'); return; } u.active = !u.active; setData('users', users); renderUsers(); };
window.deleteUser = function(id) { if (!confirm('⚠️ حذف المستخدم؟')) return; window.users = users.filter(us => us.id !== id); setData('users', users); renderUsers(); showToast('🗑️ تم الحذف', 'info'); };
...
window.renderSettings = function() {
    ...
    if ($('setCashBoxesCount') && typeof cashBoxes !== 'undefined') $('setCashBoxesCount').textContent = cashBoxes.length;
    ...
    if ($('currentUserForPassword')) $('currentUserForPassword').textContent = currentUser ? (currentUser.email || currentUser.name) : '-';
};

// ✅ changePassword - نسخة Firebase Auth الجديدة
window.changePassword = async function() {
    if (!currentUser) { showToast('⚠️ غير مسجل دخول', 'error'); return; }
    if (!currentUser.email) { showToast('⚠️ لا يوجد بريد إلكتروني', 'error'); return; }
    
    const oldP = $('oldPassword').value;
    const newP = $('newPassword').value;
    const conP = $('confirmPassword').value;
    
    if (newP.length < 6) { showToast('❌ كلمة المرور 6 أحرف على الأقل', 'error'); return; }
    if (newP !== conP) { showToast('❌ غير متطابقة', 'error'); return; }
    
    try {
        const user = firebase.auth().currentUser;
        if (!user) { showToast('❌ غير مسجل', 'error'); return; }
        ...
        if (typeof addAuditLog === 'function') addAuditLog('edit', 'user', `تغيير كلمة المرور: ${currentUser.name}`);
        showToast('✅ تم تغيير كلمة المرور بنجاح', 'success');
    } catch (e) {
        console.error('❌ خطأ تغيير كلمة المرور:', e);
        let msg = '❌ فشل تغيير كلمة المرور';
        if (e.code === 'auth/wrong-password') msg = '❌ كلمة المرور الحالية خاطئة';
        if (e.code === 'auth/weak-password') msg = '❌ كلمة المرور الجديدة ضعيفة';
        if (e.code === 'auth/requires-recent-login') msg = '⚠️ يرجى إعادة تسجيل الدخول أولاً';
        showToast(msg, 'error');
    }
};
...
window.importData = function(event) {
    ...
    reader.onload = function(e) {
        try {
            ...
            saveAll(); populateAllDropdowns(); refreshAllViews();
